...
export async function runDigestGenerationCycle() {
  ...
  try {
    ...
    for (const user of users) {
      ...
      for (const membership of user.memberships) {
        ...
        try {
          ...
          // (This updates an in-app notice tracking model used by the notification bell)
    ...
    console.error('❌ Critical system failure during background scheduled task lifecycle:', error);
  }
}

/**
 * Initializes the background scheduler.
 * Configured by default to run every hour, but easily adjusted via cron syntax variables.
 */
export function initializeScheduler() {
  // Cron syntax representing regular hourly intervals ("0 * * * *")
